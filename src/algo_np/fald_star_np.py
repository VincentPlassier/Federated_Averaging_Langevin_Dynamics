...
class FaldStar(BaseFald):
    ...
    def param_update(self):
        ...
        for i, g in enumerate(gaussians):
            ...
            grad = self.gradU[i].gradsto(self.param_clients[i]) - self.control_variates[i] + self.cv_mean
            self.param_clients[i] = self.param_clients[i] - self.gamma * grad + noise

    def control_variates_update(self):
        reference_pt = np.copy(np.mean(self.param_clients, axis=0))
        self.new_control_variates = [np.copy(gradu.grad(reference_pt)) for gradu in self.gradU]
        self.new_cv_mean = np.mean(self.new_control_variates, axis=0)

    def step(self):
        self.control_variates = np.copy(self.new_control_variates)
...
if __name__ == '__main__':
    ...
    q = .5
    # define the FaldStar sampler
    faldstar = FaldStar(gradU, param_clients, tau, pc, q, gamma)
    # define the number of iterations
    mc_iter = 10000
    # run the algorithm
    for _ in range(mc_iter):
        faldstar.step()
    param_list = faldstar.get_saved_params()
    # Display the results
