...
class BinaryLogisticGradSto:
    ...
    def __init__(self, Xtrain, Ytrain, batch_size, temperature = 1., l2 = .1, oe = None):
        """ We need y in {0,1}. """
        self.Xtrain = Xtrain
        self.Ytrain = Ytrain
    ...
    def loss(self, theta):
        ...
        return np.sum(neglogit1 + (1 - self.Ytrain) * Xtheta) + penalization

    def grad_loss(self, theta, X, Y):
        return (self.datasize / self.temperature) * np.mean(X.T * (1 / (1 + np.exp(- np.dot(X, theta))) - Y),
...
class LogisticGradSto:
    ...
    def loss(self, theta):
        ...
        return np.take_along_axis(Xtheta, self.idy, axis=1).sum() + np.sum(
            np.log(np.sum(np.exp(np.expand_dims(min_Xtheta, axis=1) - Xtheta), axis=1)) - min_Xtheta) + penalization

    def grad_loss(self, theta, X, Y):
        Xtheta = - np.dot(X, theta)
        P = softmax(Xtheta, axis=1)
        return self.datasize * X.T.dot(Y - P) / (len(Y) * self.temperature) + self.l2 * theta  # Normalized
...
Logistic = [BinaryLogisticGradSto, LogisticGradSto][0]
