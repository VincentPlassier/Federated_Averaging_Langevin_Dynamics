...
from utils.learning_rates import CosineAnnealingWarmRestarts
...
for key, value in {'m_': args.model, 'clients_': args.num_clients, 'c_': args.prob_update_param,
                   'l_': args.learning_rate}.items():  # , 'init_num_': args.init_num
    title += '-' + key + str(value)
...
path_variables = path_workdir + f'{args.method}{title.split("-s_")[0]}/'
...
# Define the number of update on each client
prob_update_param = 1 / args.num_clients if args.prob_update_param is None else args.prob_update_param
prob_update_cv = 20 * 16 / 50000  # prob_update_param
prob_communication = {'prob_update_param': prob_update_param, 'prob_update_cv': prob_update_cv}
...
param_optim = {'num_epochs': 0, 'batch_size': 64, 'lr': 0.01, 'momentum_decay': 0.9, 'weight_decay': 0.01}
lr_args = {'T_0':     args.num_iter // 10 + 1, 'T_mult': 2, 'eta_min': args.learning_rate,
           'eta_max': 1 * args.learning_rate, 'last_epoch': -1, 'max_epoch': args.num_iter}
lr_dict = {'scheduler': CosineAnnealingWarmRestarts, 'args': lr_args}

dict_algo = {'fald': 'Fald', 'vr_fald_star': 'VrFaldStar'}
algo_dl = import_module('algo_dl.' + args.method)
algorithm = getattr(algo_dl, dict_algo[args.method])

model = algorithm(testloader, trainloader, net, param_optim, load_init=False, save_init=True, path_server=path_samples)

pprint(f"--- Run {args.method.upper()} ---")

# Start the timer
startTime = time.time()

# Performs num_iter iterations of algorithm
model_state_dict, save_stats = model.run(trainloader, testloader, args.num_iter, args.weight_decay, prob_communication,
                                         lr_dict, pvals, args.burnin, args.thinning, 0, args.save_samples,
